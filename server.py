...
def headers(tree, result):
    ...
    if len(title.xpath(r'./text()')) == 1 and title.xpath(r'./a') != []:
        ...
        result['title'] = ''
        result['University'] = ''
    # result['title'] = headers.xpath(r'./div[@class = "gsc_prf_il"]/text()')[0][:-2]
    # result['University'] = headers.xpath(r'./div[@class = "gsc_prf_il"]/a/text()')[0]
...
def citations(tree, result):
    ...
    #table2 = citations.xpath(r'./div/div/div[@class = "gsc_md_hist_b"]//text()')
...
def co_authers(urlname, result):
    ...
    for each in all_coauthers:
        ...
        try:
            ...
            continue
        #author['urlname'] = each.xpath(r'./div[@class = "gsc_oai"]/h3/a/@href')[0][16:-9]
        ...
        while tree.xpath(r'//button[@id = "gsc_bpf_more"]/@disabled') == []:
            ...
            for each in papers:
                ...
                try:
                    ...
                    result['papers'][data[0]]['publication'] = data[6]
                    # print papers_info
                    #paper['authers'] = each.xpath(r'./td[@class = "gsc_a_t"]/div[@class = "gs_gray"]/text()')[0]
                    #if len(each.xpath(r'./td[@class = "gsc_a_t"]/div[@class = "gs_gray"]/text()')) == 2:
                    #    paper['publication'] = each.xpath(r'./td[@class = "gsc_a_t"]/div[@class = "gs_gray"]/text()')[1]
                    #else:
                    #    paper['publication'] = ''
                    #if each.xpath(r'./td[@class = "gsc_a_y"]/span/text()') != []:
                    #    paper['year'] = each.xpath(r'./td[@class = "gsc_a_y"]/span/text()')[0]
                    #else:
                    #    paper['year'] = ''
                    #if each.xpath(r'./td[@class = "gsc_a_c"]/a/text()') != []:
                    #    paper['citation_sum'] = each.xpath(r'./td[@class = "gsc_a_c"]/a/text()')[0]
                    #else:
                    #    paper['citation_sum'] = '0'
                    #if each.xpath(r'./td[@class = "gsc_a_c"]/a/@href') != []:
                    #    paper['all_citation_href'] = each.xpath(r'./td[@class = "gsc_a_c"]/a/@href')
                    #else:
                    #    paper['all_citation_href'] = ''
...
def crawler_venue_type(name):
    ...
    Headers = {r"accept": r"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8",
               ...
               r"User-Agent": r"Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/66.0.3359.117 Safari/537.36",
               # r"User-Agent": r"Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/67.0.3396.99 Safari/537.36",
...
def user():
    ...
    if id == 0 or id == 1:
        ...
        res = {}
        res['num'] = 0
        res['tree'] = json.loads(scholarlens_python.scholarlens(json.dumps(query)))
        ...
        f.write(scholarlens_python.scholarlens(json.dumps(query)))
        f.close()     
        ...
        query_result = json.loads(scholarlens_python.scholarlens_aligned(json.dumps(up_query), json.dumps(down_query)))
        res = {}
        res['num'] = 0
        ...
        res = {}
        res['num'] = 0
        ret_str = scholarlens_python.scholarlens(json.dumps(query))
        print(ret_str)
    ...
    #     papers_list = request.args.get('list', default=1, type=str)
    #     papers_list = json.loads(papers_list)
    #     first = request.args.get('first', default=1, type=str)
    #     second = request.args.get('second', default=1, type=str)
    #     if first == 'P. Year' and second == 'Venue':
    #         return show_details(papers_list)
    #     elif first == 'Venue' and second == 'P. Year':
    #         return show_details_venue(papers_list)
...
def readfile():
    ...
    ret = scholarlens_python.scholarlens(json.dumps(query))
    print(ret)
    if ret=="Success :)":
